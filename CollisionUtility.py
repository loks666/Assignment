...
class CollisionUtility:
    ...
    def check_lander_collision_with_surface(lander, surface):
        ...
        for i in range(len(surface_points) - 1):
            top_intersect_point = CollisionUtility.calculateIntersectPoint(lander_top_line[0], lander_top_line[1],
                                                                           surface_points[i], surface_points[i + 1])
            bottom_intersect_point = CollisionUtility.calculateIntersectPoint(lander_bottom_line[0],
                                                                              lander_bottom_line[1], surface_points[i],
                                                                              surface_points[i + 1])
            left_intersect_point = CollisionUtility.calculateIntersectPoint(lander_left_line[0], lander_left_line[1],
                                                                            surface_points[i], surface_points[i + 1])
            right_intersect_point = CollisionUtility.calculateIntersectPoint(lander_right_line[0], lander_right_line[1],
                                                                             surface_points[i], surface_points[i + 1])
            if (
                    bottom_intersect_point != None or top_intersect_point != None or left_intersect_point != None or right_intersect_point != None):
                intersection_point_found = True

        if (not intersection_point_found):
            lowest_lander_point = max(lander_bottom_line[0][1], lander_bottom_line[1][1], lander_top_line[0][1],
                                      lander_top_line[1][1])
